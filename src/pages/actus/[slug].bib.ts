 ...
 * une publication du format « actus ».
 *
 *   GET /actus/<slug>.bib
 *
 * Le bouton « BibTeX » du bloc « Pour citer cet article » pointe ici
 * via un `<a href="/actus/<slug>.bib" download>`. Content-Disposition
...
export const GET: APIRoute = async ({ params, url }) => {
  ...
  const post = await fetchBySlug<Post>('actus', slug);
  if (!post || post.draft) {
    return new Response('Not found', { status: 404 });
  }

  const articleUrl = new URL(`/actus/${post.slug}/`, url).toString();
  ...
  const body = toBibTeX(post, { articleUrl, accessedAt, siteName, collection: 'actus' });
