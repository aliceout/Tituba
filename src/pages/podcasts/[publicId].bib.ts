 ...
 * une publication du format « podcasts ».
 *
 *   GET /podcasts/<code>.bib
 *
 * Le bouton « BibTeX » du bloc « Pour citer cet article » pointe ici
 * via un `<a href="/podcasts/<code>.bib" download>`. Content-Disposition
...
export const GET: APIRoute = async ({ params, url }) => {
  ...
  const post = await fetchByPublicId<Post>('podcasts', publicId);
  if (!post || post.draft) {
    return new Response('Not found', { status: 404 });
  }

  const articleUrl = new URL(`/podcasts/${post.publicId}/`, url).toString();
  ...
  const body = toBibTeX(post, { articleUrl, accessedAt, siteName, collection: 'podcasts' });
  ...
  return new Response(body, {
    ...
    headers: {
      ...
      'Content-Disposition': `attachment; filename="podcasts-${post.publicId}.bib"`,
