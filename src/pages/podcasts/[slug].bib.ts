 ...
 *
 *   GET /podcasts/<slug>.bib
 *
 * Le bouton « BibTeX » du bloc « Pour citer cet article » pointe ici
 * via un `<a href="/podcasts/<slug>.bib" download>`. Content-Disposition
...
export const GET: APIRoute = async ({ params, url }) => {
  ...
  const post = await fetchBySlug<Post>('podcasts', slug);
  if (!post || post.draft) {
    return new Response('Not found', { status: 404 });
  }

  const articleUrl = new URL(`/podcasts/${post.slug}/`, url).toString();
